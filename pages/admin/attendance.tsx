...
import { CalendarDays, Users, ClipboardCheck, TriangleAlert, TrendingUp, FileText } from 'lucide-react'
...
const statusOrder: AttendanceRecord['status'][] = ['present', 'late', 'absent', 'leave']
...
export default function AdminAttendancePage() {
  ...
  const stats = useMemo<Array<{ title: string; value: string | number }>>(() => ([
    ...
    { title: 'Pending Balance', value: summaryValues.pending_balance ? `KES ${summaryValues.pending_balance}` : 'KES 0' },
  ]), [summaryValues])

  const followUps = useMemo(() => {
    return students
      .filter((student) => (student.attendance_rate || 0) < 85)
      .map((student) => ({
        id: student.id,
        name: student.name,
  ...
  const refreshAll = async () => {
    ...
    const [contextResponse, historyResponse] = await Promise.all([
      requestJson<AttendanceContextResponse>(`/api/dashboard/attendance/context/?${query.toString()}`),
      requestJson<AttendanceHistoryResponse>(`/api/dashboard/attendance/records/?page=${historyPage}&page_size=8&school_class_id=${selectedClassId}&date=${selectedDate}`),
    ])

    setClasses(contextResponse.classes)
    setStudents(contextResponse.students)
    setRecords(contextResponse.records)
    setSummaryData(contextResponse.summary)
    setStatusMap(contextResponse.students.reduce<Record<string, AttendanceRecord['status']>>((acc, student) => {
      const existing = contextResponse.records.find((record) => record.student_id === student.id)
      acc[student.id] = existing?.status || 'present'
      return acc
    }, {}))
    setHistory(historyResponse.records)
    setHistoryTotalPages(historyResponse.total_pages || 1)
  }
  ...
  const openAttendancePdf = async () => {
    const fallbackUrl = `${window.location.origin}/api/reports/attendance/pdf/`
    ...
    <Layout role="admin">
      ...
      <div className="md:col-span-3 card p-6">
        <div className="flex items-center justify-between gap-4 mb-4">
          ...
          <button type="button" onClick={openAttendancePdf} className="text-xs font-semibold text-green-700 bg-green-50 px-3 py-1.5 rounded hover:bg-green-100">Open Attendance PDF</button>
        </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-4 mb-5">
          ...
          <label className="flex flex-col gap-2">
            ...
            <input type="date" value={selectedDate} onChange={(event) => setSelectedDate(event.target.value)} className="h-11 rounded-lg border border-gray-200 bg-white px-3 text-sm text-gray-900" />
          </label>
        ...
        <form onSubmit={handleSave} className="space-y-3">
          {students.length === 0 ? (
            <p className="text-sm text-gray-500">Select a class to load attendance rows.</p>
          ) : students.map((student) => (
            <div key={student.id} className="rounded-lg border border-gray-100 p-3 flex flex-col gap-3 md:flex-row md:items-center md:justify-between">
              <div>
                <p className="font-semibold text-gray-900">{student.name}</p>
                <p className="text-xs text-gray-500">{student.admission_number} • {student.class_name}</p>
                <p className="text-xs text-gray-400 mt-1">Attendance rate: {student.attendance_rate}%</p>
              </div>
              <div className="flex flex-wrap items-center gap-2">
                {statusOrder.map((status) => (
                  <button
                    ...
                    className={`px-3 py-1.5 rounded-full text-xs font-semibold border ${statusMap[student.id] === status ? statusStyles[status] : 'bg-white text-gray-600 border-gray-200'}`}
                  >
                    {status}
                  </button>
                ))}
          ...
          <div className="flex flex-wrap gap-3 items-center pt-2">
            ...
            {summaryData && <span className="text-xs text-gray-500">Present: {summaryData.present} • Late: {summaryData.late} • Absent: {summaryData.absent} • Leave: {summaryData.leave} • Rate: {summaryData.attendance_rate}%</span>}
          </div>
      ...
      <div className="md:col-span-2 card p-6">
        <div className="flex items-center gap-2 mb-4">
          ...
          <h3 className="text-lg font-semibold text-gray-900">Saved Register</h3>
        </div>
        <div className="space-y-3">
          {records.length === 0 ? (
            <p className="text-sm text-gray-500">No saved records for this class and date.</p>
          ) : records.map((record) => (
            <div key={record.id} className="rounded-lg border border-gray-100 p-3">
              <div className="flex items-start justify-between gap-3">
                ...
                <span className={`px-2 py-1 rounded-full text-xs font-semibold ${statusStyles[record.status]}`}>{record.status}</span>
              </div>
              <div className="mt-3 flex gap-2 justify-end">
                <button type="button" onClick={() => setStatusMap((current) => ({ ...current, [record.student_id]: record.status }))} className="text-xs font-semibold text-green-700 bg-green-50 px-3 py-1.5 rounded hover:bg-green-100">Sync</button>
                <button type="button" onClick={() => handleDelete(record.id)} className="text-xs font-semibold text-red-700 bg-red-50 px-3 py-1.5 rounded hover:bg-red-100">Delete</button>
              </div>
            </div>
          ))}
        </div>
      ...
      <div className="card p-6">
        <div className="flex items-center gap-2 mb-4">
          ...
          <h3 className="text-lg font-semibold text-gray-900">At-Risk Attendance</h3>
        </div>
        <div className="space-y-3">
          {followUps.length === 0 ? (
            <p className="text-sm text-gray-500">No follow-up needed. All visible students are above the threshold.</p>
          ) : followUps.map((student) => (
            <div key={student.id} className="flex items-start justify-between gap-3 rounded-lg border border-yellow-100 bg-yellow-50/60 p-3">
              <div>
                <p className="font-semibold text-gray-900">{student.name}</p>
                <p className="text-xs text-gray-500">{student.admission_number} • {student.class_name}</p>
              </div>
              <div className="text-right">
                <p className="text-sm font-semibold text-yellow-800">{student.rate}% attendance</p>
                <p className="text-xs text-yellow-700">Follow up with parent/guardian</p>
              </div>
            </div>
          ))}
        </div>
          ...
          {history.length === 0 ? (
            ...
            <div key={record.id} className="p-3 border border-gray-100 rounded-lg hover:bg-gray-50 transition-colors">
              <div className="flex items-start justify-between gap-3">
                <div>
                  <p className="font-semibold text-gray-900">{record.student_name}</p>
                  <p className="text-xs text-gray-500">{record.admission_number} • {record.date.slice(0, 10)} • {record.school_class}</p>
                </div>
                <span className={`px-2 py-1 rounded-full text-xs font-semibold ${statusStyles[record.status]}`}>{record.status}</span>
              </div>
            </div>
