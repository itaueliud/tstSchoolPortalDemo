...
export default function TeacherTimetablePage() {
  ...
  const loadTimetable = async (targetWeekStart = weekStart) => {
  ...
  const handleCreateEntry = async (event: FormEvent<HTMLFormElement>) => {
    ...
    try {
      await requestJson('/api/dashboard/timetable/', {
        method: 'POST',
  ...
  const handleDownload = async () => {
    try {
      ...
      anchor.href = url
      anchor.download = `teacher-timetable-${weekStart}.csv`
      anchor.click()
    ...
    <Layout role="teacher">
      ...
      <div className="md:col-span-3 card p-6">
        <div className="flex items-center justify-between gap-3 flex-wrap mb-4">
          ...
          <div className="flex items-center gap-2">
            ...
            <button type="button" onClick={handleDownload} className="inline-flex items-center gap-2 rounded-lg bg-green-600 px-4 py-2 text-sm font-semibold text-white hover:bg-green-700">
        ...
        <div className="grid grid-cols-1 md:grid-cols-4 gap-4 mb-5">
          ...
          <div className="md:col-span-3 grid grid-cols-2 sm:grid-cols-4 gap-3">
            ...
            <div className="rounded-lg border border-gray-100 bg-amber-50 p-3">
              ...
              <p className="text-2xl font-bold text-gray-900">{summaryData.pending}</p>
            </div>
            <div className="rounded-lg border border-gray-100 bg-rose-50 p-3">
              <p className="text-xs text-gray-500">Needs Review</p>
              <p className="text-2xl font-bold text-gray-900">{summaryData.missed + summaryData.rescheduled}</p>
            </div>
          </div>
        </div>
        ...
        <form onSubmit={handleCreateEntry} className="rounded-xl border border-gray-100 bg-green-50/40 p-4 space-y-4">
          ...
          <div className="flex items-center gap-3">
            <button type="submit" disabled={saving || !form.school_class_id || !form.subject.trim()} className="inline-flex items-center gap-2 rounded-lg bg-green-600 px-5 py-2.5 text-sm font-semibold text-white hover:bg-green-700 disabled:opacity-70">
        ...
        <div className="mt-5 space-y-3">
          {entries.length === 0 ? (
            <div className="rounded-xl border border-dashed border-gray-200 p-6 text-sm text-gray-500">
              No timetable entries for this week yet. Add the first class block above.
            </div>
          ) : entries.map((entry) => {
            const draft = drafts[entry.id] || { status: entry.status, notes: entry.notes }
            return (
      ...
      <div className="card p-6">
        <div className="flex items-center gap-2 mb-4">
          <CalendarDays className="w-5 h-5 text-green-600" />
          <h3 className="text-lg font-semibold text-gray-900">Timetable Overview</h3>
        </div>
        <div className="space-y-3">
          <div className="rounded-lg border border-gray-100 bg-green-50 p-4">
            <p className="font-semibold text-gray-900">The timetable is stored online</p>
            <p className="text-sm text-gray-600 mt-1">Teachers can add weekly lessons, mark them as completed or rescheduled, and export a CSV copy of the current week.</p>
          </div>
          <div className="rounded-lg border border-gray-100 p-4">
            <p className="text-sm text-gray-500">Available classes</p>
            <div className="mt-2 space-y-2">
              {classes.length === 0 ? (
                <p className="text-sm text-gray-500">No assigned classes found for this teacher yet.</p>
              ) : classes.map((item) => (
                <div key={item.id} className="flex items-center justify-between rounded-lg bg-gray-50 px-3 py-2 text-sm">
                  <span className="font-medium text-gray-800">{item.name}</span>
                  <span className="text-gray-500">{item.grade_level || item.room || 'Assigned class'}</span>
                </div>
              ))}
            </div>
          </div>
